...
const defaultInput = `<h1 style="text-align:center; color:aqua;">DICTIONARY</h1>
<div class="container">
<div class = "row">
    <input type="text" class="col-10" id="inputText" placeholder="Search for a word ">
...
function displayException(resultJson) {
  ...
  containerRow.innerHTML =
    containerRow.innerHTML + `<div class = "col-12">${resultJson.title}</div>`;
}
...
function displayResult(resultJson) {
  ...
  containerRow.innerHTML =
    ...
    <div class = "col-2">Meaning:</div><div class = "col-10"></div>` +
    displayMeaning(resultJson.meanings);
...
function displayMeaning(meaningArray) {
  ...
  meaningArray.forEach((e) => {
    returnVal =
      returnVal +
      `<div class="col-2">${e.partOfSpeech}</div>
